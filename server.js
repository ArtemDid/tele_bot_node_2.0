...
bot.on(/^\d{2}(.)\d{2}\1\d{4}$/, (msg) => {
    ...
    axios({
        ...
        .then(response => {
            ...
            const createQuery = `INSERT INTO history (query, user_name, date, answer) VALUES ($1, $2, $3, $4) RETURNING *`;
            const values = [
                msg.text,
                msg.from.first_name,
                date.toISOString(),
...
server.get('/', (req, res) => {
    ...
    const createQuery = 'SELECT * FROM public.history ORDER BY "id" ASC';
    const values = null;
